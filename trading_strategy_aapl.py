...
for i in range(1, len(df)):
    try:
        ...
        bb_upper_val = float(df['BB_Upper'].iloc[i])
        bb_lower_val = float(df['BB_Lower'].iloc[i])
        
        # NaN 체크
        if pd.isna(rsi_val) or pd.isna(signal_val) or pd.isna(ma50_val) or pd.isna(bb_lower_val):
            continue
        
        # 간단한 매수 조건 (더 관대하게 설정)
        # RSI 과매도 + 가격이 하단 볼린저 근처
        if (rsi_val < 40 and close_val < bb_lower_val * 1.05):
            df.at[df.index[i], 'Buy_Signal'] = True
        
        # 간단한 매도 조건
        # RSI 과매수 + 가격이 상단 볼린저 근처
        if (rsi_val > 60 and close_val > bb_upper_val * 0.95):
...
if len(buy_signals) > 0:
    ...
    for idx, row in buy_signals.iterrows():
        print(f"  {idx.date()} - 가격: ${row['Close']:.2f}, RSI: {row['RSI']:.2f}, MACD: {row['MACD']:.4f}")
else:
    print("\n✅ 매수 신호: 없음")
...
if len(buy_points) > 0:
    ...
    for idx, row in buy_points.iterrows():
        ax1.text(idx, row['Close'] + 5, 'BUY', fontsize=9, color='green', fontweight='bold', 
...
if len(sell_points) > 0:
    ...
    for idx, row in sell_points.iterrows():
        ax1.text(idx, row['Close'] - 5, 'SELL', fontsize=9, color='red', fontweight='bold', 
                ha='center', bbox=dict(boxstyle='round,pad=0.3', facecolor='lightcoral', alpha=0.7))

ax1.set_title('AAPL 가격 + 이동평균 + 거래 신호', fontsize=14, fontweight='bold')
